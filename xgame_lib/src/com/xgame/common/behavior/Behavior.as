package com.xgame.common.behavior
{
	...
	public class Behavior implements IEventDispatcher
	{
		...
		{
			_halfSceneWidth = UIManager.stage.stageWidth / 2;
			_halfSceneHeight = UIManager.stage.stageHeight / 2;
		...
		protected function calculatePosition(): void
		{
			...
			if(Camera.instance.focus == _target)
			{
				...
				targetX = _target.positionX > (Map.MapSize.x - _halfSceneWidth) ? (_target.positionX - (Map.MapSize.x - UIManager.stage.stageWidth)) : targetX;
				targetY = _target.positionY > (Map.MapSize.y - _halfSceneHeight) ? (_target.positionY - (Map.MapSize.y - UIManager.stage.stageHeight)) : targetY;
			}
			else
			{
