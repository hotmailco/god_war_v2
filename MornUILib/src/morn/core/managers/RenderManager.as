...
package morn.core.managers {
	...
	public class RenderManager {
		...
		private function invalidate():void {
			UIManager.stage.addEventListener(Event.RENDER, onValidate);
			//render有一定几率无法触发，这里加上保险处理
			UIManager.stage.addEventListener(Event.ENTER_FRAME, onValidate);
			if (UIManager.stage) {
				UIManager.stage.invalidate();
			}
		}
		
		private function onValidate(e:Event):void {
			UIManager.stage.removeEventListener(Event.RENDER, onValidate);
			UIManager.stage.removeEventListener(Event.ENTER_FRAME, onValidate);
			renderAll();
			UIManager.stage.dispatchEvent(new Event(UIEvent.RENDER_COMPLETED));
		}
