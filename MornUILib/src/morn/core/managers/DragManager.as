	...
	public class DragManager {
		...
		public function doDrag(dragInitiator:Sprite, dragImage:Sprite = null, data:* = null, offset:Point = null):void {
			...
			if (_dragImage != _dragInitiator) {
				if (offset) {
					_dragImage.x = UIManager.stage.mouseX - offset.x;
					_dragImage.y = UIManager.stage.mouseY - offset.y;
				} else {
					var p:Point = _dragInitiator.localToGlobal(new Point());
					_dragImage.x = p.x;
					_dragImage.y = p.y;
				}
				UIManager.stage.addChild(_dragImage);
			}
			_dragImage.startDrag();
			_dragInitiator.dispatchEvent(new DragEvent(DragEvent.DRAG_START, dragInitiator, data));
			UIManager.stage.addEventListener(MouseEvent.MOUSE_MOVE, onStageMouseMove);
			UIManager.stage.addEventListener(MouseEvent.MOUSE_UP, onStageMouseUp);
		}
		
		/**放置把拖动条拖出显示区域*/
		private function onStageMouseMove(e:MouseEvent):void {
			if (e.stageX <= 0 || e.stageX >= UIManager.stage.stageWidth || e.stageY <= 0 || e.stageY >= UIManager.stage.stageHeight) {
				_dragImage.stopDrag();
			} else {
				_dragImage.startDrag();
			}
		...
		private function onStageMouseUp(e:Event):void {
			UIManager.stage.removeEventListener(MouseEvent.MOUSE_MOVE, onStageMouseMove);
			...
			if (_dragInitiator != _dragImage) {
				if (UIManager.stage.contains(_dragImage)) {
					UIManager.stage.removeChild(_dragImage);
				}
			}
			_dragInitiator = null;
			_data = null;
