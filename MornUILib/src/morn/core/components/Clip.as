...
package morn.core.components {
	...
	public class Clip extends Component implements IClip {
		...
		protected function changeClip():void {
			if (UIManager.asset.hasClass(_url)) {
				loadComplete(_url, UIManager.asset.getBitmapData(_url, false));
			} else {
				UIManager.loader.loadBMD(_url, new Handler(loadComplete, [_url]));
			}
		}
		...
		protected function loadComplete(url:String, bmd:BitmapData):void {
			if (url == _url && bmd) {
				...
				UIManager.asset.cacheBitmapData(url, bmd);
				clips = UIManager.asset.getClips(url, _clipX, _clipY);
				UIManager.asset.disposeBitmapData(url);
			}
		}
		...
		public function play():void {
			_isPlaying = true;
			frame = _bitmap.index;
			UIManager.timer.doLoop(_interval, loop);
		}
		...
		public function stop():void {
			UIManager.timer.clearTimer(loop);
			_isPlaying = false;
		}
		...
		public function dispose(clearFromLoader:Boolean = false):void {
			...
			UIManager.asset.destroyClips(_url);
			if (clearFromLoader) {
				UIManager.loader.clearResLoaded(_url);
			}
		}
	}
}