		...
		public function set isOpen(value:Boolean):void {
			if (_isOpen != value) {
				...
				if (_isOpen) {
					...
					UIManager.stage.addChild(_list);
					UIManager.stage.addEventListener(MouseEvent.MOUSE_DOWN, removeList);
					UIManager.stage.addEventListener(Event.REMOVED_FROM_STAGE, removeList);
					//处理定位
					_list.scrollTo((_selectedIndex + _visibleNum) < _list.length ? _selectedIndex : _list.length - _visibleNum);
				} else {
					_list.remove();
					UIManager.stage.removeEventListener(MouseEvent.MOUSE_DOWN, removeList);
					UIManager.stage.removeEventListener(Event.REMOVED_FROM_STAGE, removeList);
				}
			}
		}
