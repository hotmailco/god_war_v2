...
package morn.core.components {
	...
	public class ScrollBar extends Component {
		...
		protected function onButtonMouseDown(e:MouseEvent):void {
			...
			slide(isUp);
			UIManager.timer.doOnce(Styles.scrollBarDelayTime, startLoop, [isUp]);
			UIManager.stage.addEventListener(MouseEvent.MOUSE_UP, onStageMouseUp);
		}
		
		protected function startLoop(isUp:Boolean):void {
			UIManager.timer.doFrameLoop(1, slide, [isUp]);
		}
		...
		protected function onStageMouseUp(e:MouseEvent):void {
			UIManager.stage.removeEventListener(MouseEvent.MOUSE_UP, onStageMouseUp);
			UIManager.timer.clearTimer(startLoop);
			UIManager.timer.clearTimer(slide);
		}
		...
		protected function onTargetMouseDown(e:MouseEvent):void {
			...
			UIManager.timer.clearTimer(tweenMove);
			if (!this.contains(e.target as DisplayObject)) {
				UIManager.stage.addEventListener(MouseEvent.MOUSE_UP, onStageMouseUp2);
				UIManager.stage.addEventListener(Event.ENTER_FRAME, onStageEnterFrame);
				_lastPoint = new Point(UIManager.stage.mouseX, UIManager.stage.mouseY);
			}
		}
		
		protected function onStageEnterFrame(e:Event):void {
			_lastOffset = _slider.direction == VERTICAL ? UIManager.stage.mouseY - _lastPoint.y : UIManager.stage.mouseX - _lastPoint.x;
			if (Math.abs(_lastOffset) >= 1) {
				_lastPoint.x = UIManager.stage.mouseX;
				_lastPoint.y = UIManager.stage.mouseY;
		...
		protected function onStageMouseUp2(e:MouseEvent):void {
			UIManager.stage.removeEventListener(MouseEvent.MOUSE_UP, onStageMouseUp2);
			UIManager.stage.removeEventListener(Event.ENTER_FRAME, onStageEnterFrame);
			if (Math.abs(_lastOffset) > 50) {
				_lastOffset = 50 * (_lastOffset > 0 ? 1 : -1);
			}
			UIManager.timer.doFrameLoop(1, tweenMove);
		}
